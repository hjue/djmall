...
class Image(models.Model):
    ...
    modify_time = models.DateField(
        verbose_name='修改时间', auto_now=True, null=True)
    # 是否删除
    is_delete = models.BooleanField(
        verbose_name='是否删除', default=False, null=True)
    ...
    def natural_key(self):
        # 获取图片地址的时候自动添加服务器地址
        return ('http://127.0.0.1:8001/' + str(self.src))
...
class Categorys(models.Model):
    ...
    src = models.ImageField(
        verbose_name='分类图片', upload_to='upload/categorys', null=True, blank=True)
...
class Sku(models.Model):
    ...
    # 商品(Spu)
    id_goods = models.ForeignKey(
        to=Goods, verbose_name='商品(Spu)', on_delete=models.SET_NULL, null=True)
    # 商品属性键值对[属性名:属性值，多个属性使用"逗号"隔开]
    attribute = models.TextField(
        verbose_name='属性名:属性值，多个属性使用"逗号"隔开', null=True)
    # 市场价
    market_price = models.DecimalField(
        verbose_name='市场价', max_digits=10, decimal_places=2)
    # 真实价格
    price = models.DecimalField(
        verbose_name='真实价格', max_digits=10, decimal_places=2)
    # 商品库存
    stock = models.IntegerField(verbose_name='商品库存')
